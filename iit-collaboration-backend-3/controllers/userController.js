...
export const myLikedRepos = catchAsyncError(async (req, res, next) => {
    const user = await User.findById(req.user._id);
    console.log({ user });
    const repos = [];

    const myLikedRepos = await UploadFile.find({ _id: { $in: user.myLikes } });

    // console.log({ myLikedRepos });
...
export const getIdByMail = catchAsyncError(async (req, res, next) => {
